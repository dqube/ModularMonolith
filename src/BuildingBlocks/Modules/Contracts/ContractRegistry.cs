...
namespace CompanyName.MyProjectName.BuildingBlocks.Modules.Contracts;
...
public class ContractRegistry : IContractRegistry
{
    ...
    public IContractRegistry Register<T>()
        where T : class
    {
    ...
    public IContractRegistry RegisterPathWithRequest<TRequest>(string path)
        where TRequest : class
        => RegisterPath<TRequest, Empty>(path);

    public IContractRegistry RegisterPathWithResponse<TResponse>(string path)
    ...
    private static Type GetContractType<T>()
        where T : class
        => typeof(T);
    ...
    private void ValidateContract(Type contractType, string path = null)
    {
        ...
        var originalContract = Activator.CreateInstance(originalType);
        var originalContractType = originalContract.GetType();
        foreach (var propertyName in contract.Required)
        {
            var localProperty = GetProperty(contract.Type, propertyName, contractName, module, contractModule, path);
            var originalProperty = GetProperty(originalContractType, propertyName, contractName, module, contractModule, path);
            ValidateProperty(localProperty, originalProperty, propertyName, contractName, module, contractModule, path);
        }
...
#pragma warning restore SA1122 // Use string.Empty for empty strings
    ...
    private static PropertyInfo GetProperty(Type type, string name, string contractName, string module, string localModule, string path = null)
    {
