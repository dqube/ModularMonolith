...
internal sealed class EfOutbox<T> : IOutbox
    where T : DbContext
{
    ...
    public async Task SaveAsync<TMessage>(MessageEnvelope<TMessage> message, CancellationToken cancellationToken = default)
    ...
    private async Task SendMessageAsync(OutboxMessage outboxMessage, CancellationToken cancellationToken)
    {
        ...
        _logger.LogInformation(
            "Sending a message from outbox: {Name} [Message ID: {MessageId}, Activity ID: {ActivityId}]...", name, messageId, context.ActivityId);
